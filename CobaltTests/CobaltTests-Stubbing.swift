...
class CobaltTestsStubbing: CobaltTests {
    ...
    lazy var stubClient: CobaltClient = {
        let stubConfig = config
        stubConfig.host = "https://e-sites.nl"
        return CobaltClient(config: stubConfig)
    }()
    
    override func tearDown() {
        super.tearDown()
        stubClient.stubbing.removeAll()
    }
    
    override func setUp() {
        super.setUp()
        stubClient.stubbing.register()
    }
...
"""#
        ...
        waitUntil { done in
            ...
            self.stubClient.request(request).sink(receiveCompletion: { event in
                switch event {
                case .finished:
                    break
